...
def add_watermark(image_path, out_path, watermarks):
    ...
    for index, xy in enumerate(pos):
        drawer.text(
            xy=xy, text=watermarks[index],encoding='utf-8-sig', font=font, fill=config['rgba'])
        print(watermarks[index])
...
if __name__ == '__main__':
   ...
   # batch_add_waterwalk("input_dir/","output_dir/", ["测试水印"])
    ...
    # 给单个图片加水印
    lujing = 'e:/photo/微信图片_20180123123223.jpg'
    lujing1 = 'e:/photo/d333d.png'
    lujing2 = 'e:/photo/d333d333.png'
    add_watermark(lujing,lujing1,["测试水印"])

   #压缩照片
    sImg = Image.open(lujing1)
    w, h = sImg.size
    width = int(w / 2)
    height = int(h / 2)

...
def analyFemale(df_friends):
    ...
    plot.imshow(wordcloud, interpolation='bilinear')
    plot.axis("off")
    plot.savefig("analyFemale.png")
    plot.show()

# 男 女性个性签名统计
def analyFemaleMale(df_friends):
    signature = df_friends['Signature']
    signature = signature.unique()
    signature = "".join(signature)
    wordlist_after_jieba = jieba.cut(signature, cut_all=True)
    wl_space_split = " ".join(wordlist_after_jieba)
    font = os.path.join(os.path.dirname(__file__), "simhei.ttf")
    wordcloud = WordCloud(font_path=font, max_words=400, max_font_size=50, margin=2).generate(wl_space_split)
    plot.figure()
    plot.imshow(wordcloud, interpolation='bilinear')
    plot.axis("off")
    plot.savefig("analyFemaleMale.png")
    plot.show()
...
def main():
    ...
    # aa=getRoomList()
    # print(aa)
    roomMsg = getRoomMsg(u'监测站')  # 获取指定群聊的信息
    ...
    #print(df_friends.columns.values.tolist())
    #df_friends.to_csv('aabb.csv',encoding='utf-8')
